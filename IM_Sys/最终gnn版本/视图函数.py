...
def answer(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            print(f"Current user answers: {user_answers}")
                ...
                if "2" in question:  # 查药方
                    ...
                    if "感冒" in user_answers:  # 检查用户是否输入了感冒
                        if current_question_index1 < len(second_series_questions):  # 提问第二系列问题
                            ...
                            # 匹配药方
                            kbqa = KBQA4()
                            matched_prescription = kbqa.get_prescription(user_answers)
                            if matched_prescription:
                                prescription_info = f"{'药方'}: {matched_prescription['预测处方']}<br>{'置信度'}: {matched_prescription['置信度']}<br>{'主治'}: {matched_prescription['预测症状']}<br>{'方剂类型'}: {matched_prescription['方剂类型']}<br>{'用法'}: {matched_prescription['用法']}<br>{'组成'}: {', '.join(matched_prescription['处方组成'])}"
                    ...
                    user_answers.append(question)
                    print(f"Updated user answers: {user_answers}")
