...
data_dir="UserApp/QA/data/"
class EntityExtractor:
    def __init__(self):
        cur_dir = '/'.join(os.path.abspath(__file__).split('/')[:-1])
        # 路径
        self.vocab_path = os.path.join(cur_dir, 'UserApp/QA/data/vocab.txt')#UserApp/QA/data/vocab.txt
        #self.stopwords_path =os.path.join(cur_dir, 'UserApp/QA/data/stop_words.utf8')
        self.stopwords_path = os.path.join(cur_dir, 'data', 'stop_words.utf8')
        self.word2vec_path = os.path.join(cur_dir, 'UserApp/QA/data/merge_sgns_bigram_char300.txt')
        ...
        # 意图分类模型文件
        self.tfidf_path = os.path.join(cur_dir, 'model/tfidf_model.m')#UserApp/QA/model/tfidf_model.m
        self.nb_path = os.path.join(cur_dir, 'model/intent_reg_model.m')  #朴素贝叶斯模型UserApp/QA/model/intent_reg_model.m
