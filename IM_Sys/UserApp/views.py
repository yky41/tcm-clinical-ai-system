...
def register_online(request):  # 线上挂号
    if request.method == "GET":
        ...
        return render(request, "register_online.html",
                      {"user": user, "departments": departments, 'active_menu': 'register_online',
...
def get_doctors_by_department(request, department_id):
    doctors = Doctor.objects.filter(deid_id=department_id).values('uid', 'doctor_name')
...
def add_prescription(request):
    # 从会话数据中获取药方列表
    if 'session_prescriptions' in request.session:
        prescriptions = request.session['session_prescriptions']
    else:
        prescriptions = []
...
def reset_state(request):
    global current_question_index, is_prescription_query, user_answers, current_question_index1,current_question_index2, additional_prescription_query, new_prescription_step, new_prescription
    current_question_index = 0
    current_question_index1 = 0
    current_question_index2 = 0
    is_prescription_query = False
    ...
    new_prescription = {'方名': '', '主治': '', '组成': '', '方剂类型': '', '用法': ''}
    user_answers = []
    request.session['session_prescriptions'] = []
...
class PrescriptionSaver:
    ...
    def save_prescription(self, prescription):
        try:
            ...
            # 创建关系
            rel = Relationship(prescription_node, "方剂类型", prescription_type_node)
            self.graph.create(rel)
            rel = Relationship(prescription_node, "组成", composition_node)
            self.graph.create(rel)
...
def answer(request):
    ...
    if 'session_prescriptions' not in request.session:
        request.session['session_prescriptions'] = []
        ...
        try:
            ...
            if question:
                    ...
                    if matched_prescriptions:
                        ...
                        for prescription in matched_prescriptions:
                            if prescription['药方'] not in seen_prescriptions:
                                ...
                                # 记录生成的药方
                                session_prescriptions = request.session['session_prescriptions']
                                session_prescriptions.append(prescription)
                                request.session['session_prescriptions'] = session_prescriptions
                ...
                if "2" in question:  # 查药方
                    ...
                    elif "消化不良" in user_answers:  # 检查用户是否输入了感冒
                        if current_question_index2 < len(third_series_questions):  # 提问第三系列问题
                            answer = third_series_questions[current_question_index2]
                            current_question_index2 += 1
                        else:
                            matched_prescriptions = KBQA1.filter_prescription_by_indications(user_answers)
                            if matched_prescriptions:
                                prescriptions_info = ""
                    ...
                    if not answer:
                        ...
                        if prescription_info:
                            ...
                            # 记录生成的药方
                            session_prescriptions = request.session['session_prescriptions']
                            session_prescriptions.append(prescription_info)
                            request.session['session_prescriptions'] = session_prescriptions
                        else:
                            answer = "很抱歉，没有找到匹配的药方。"
                    is_prescription_query = True
                ...
                #         reset_state(request)
                    ...
                    elif "消化不良" in user_answers:
                        if current_question_index2 < len(third_series_questions):
                            answer = third_series_questions[current_question_index2]
                            current_question_index2 += 1
                        else:
                            matched_prescriptions = KBQA1.filter_prescription_by_indications(user_answers)
